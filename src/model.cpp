...
int main(int argc, char *argv[]) 
{
  long i;
  long nbSign;
  bool hybridFlag = false;
  bool onebitFlag = false;
  ...
  desc.add_options()
    ("help", "help message")
    ("hybrid", "launch the hybrid attack")
    ("onebit", "launch the onebit attack")
  ...
  if (vm.count("help") || (!hybridFlag && !onebitFlag))
  {
    std::cout << desc << "\n";
    return EXIT_SUCCESS;
  }
  ...
  {
    // получение значений
    sign.signMessage(key.pk, key.sk, message, sign_flip); // ПОДПИСЫВАНИЕ
    ...
    if(i % 5000 == 0) 
    {
        ...
        gradasc(vecv1d, matw.topLeftCorner(i+1, N), s1norm); // градиентный подъем
        
        for(int j = 0; j < N; j++) 
        {
            vecv1d[j] = std::round(vecv1d[j]); // округление к ближайшему целому
        }
        vecv1 = vecv1d.template cast<long>();
        
        matchs1 = 0;
        
        for(int j = 0; j < N; j++) // проверка сколько битов угадано
        {
  ...
  std::cout << " ONE-BIT TIME ATTACK:" << std::endl;
  std::cout << " need for full match: " << fullmatch << std::endl;
  std::cout << " need for part match: " << partmatch << std::endl;
  std::cout << "\n";
  }
